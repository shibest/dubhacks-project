...
export default function GuiltyPleasureChat() {
  const navigate = useNavigate();
  const [messages, setMessages] = useState<Message[]>([
    {
      id: '1',
      text: 'Welcome to Guilty Pleasure Guess! Share that song nobody knows you listen to! 🤫',
      sender: 'other',
      timestamp: new Date(),
      type: 'text'
    }
  ...
  const loadChatLogs = () => {
    const logs = localStorage.getItem('guiltypleasure_chat_logs');
    return logs ? JSON.parse(logs) : [];
  };

  // Save chat logs to localStorage
  const saveChatLogs = (logs: any[]) => {
    localStorage.setItem('guiltypleasure_chat_logs', JSON.stringify(logs));
  };
  ...
  const saveCurrentConversation = () => {
    if (messages.length > 1) { // Only save if there are actual messages beyond welcome
      ...
      const conversation = {
        ...
        messages: messages,
        summary: `Guilty Pleasure session with ${messages.length - 1} messages`
      };
  ...
  const handleSendMessage = () => {
    if (newMessage.trim()) {
      ...
      setTimeout(() => {
        const responses = [
          "OMG, I can't believe you listen to that! 😂",
          "That's actually kinda catchy though... 🎵",
          "My guilty pleasure is even worse! 😅",
          "Wait, that song is a bop! 🎶",
  ...
  return (
    <div className={`flex flex-col h-screen bg-gradient-to-b from-purple-50 via-pink-50 to-red-50 transition-opacity duration-500 ${
      isVisible ? "opacity-100" : "opacity-0"
    }`}>
      {/* Header */}
      <div className="bg-gradient-to-r from-purple-500 via-pink-500 to-red-600 text-white p-4 shadow-lg">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="font-bold text-lg">Guilty Pleasure Guess</h1>
                <p className="text-sm opacity-90">Share your secret jams!</p>
              </div>
            </div>
          </div>
            ...
            >
              <div
                className={`max-w-xs lg:max-w-md px-4 py-2 rounded-2xl shadow-md ${
                  message.sender === 'user'
                    ? 'bg-gradient-to-r from-purple-500 via-pink-500 to-red-600 text-gray-900 rounded-br-sm'
        ...
        <div className="flex items-end gap-2">
          ...
          <div className="flex-1 relative">
            <textarea
              ...
              placeholder="Share your guilty pleasure..."
              className="w-full px-4 py-3 pr-12 bg-gray-50 border border-gray-200 rounded-2xl resize-none focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent"
              rows={1}
          ...
          <button
            ...
            className={`p-3 rounded-full transition-all duration-200 ${
              newMessage.trim()
                ? 'bg-gradient-to-r from-purple-500 via-pink-500 to-red-600 text-white shadow-lg hover:shadow-xl'
