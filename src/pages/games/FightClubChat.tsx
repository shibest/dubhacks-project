...
export default function FightClubChat() {
  const navigate = useNavigate();
  const [messages, setMessages] = useState<Message[]>([
    {
      id: '1',
      text: 'Welcome to Fight Club! Who would win between characters from our shared games? 🥊',
      sender: 'other',
      timestamp: new Date(),
      type: 'text'
    }
  ...
  const loadChatLogs = () => {
    const logs = localStorage.getItem('fightclub_chat_logs');
    return logs ? JSON.parse(logs) : [];
  };

  // Save chat logs to localStorage
  const saveChatLogs = (logs: any[]) => {
    localStorage.setItem('fightclub_chat_logs', JSON.stringify(logs));
  };
  ...
  const saveCurrentConversation = () => {
    if (messages.length > 1) { // Only save if there are actual messages beyond welcome
      ...
      const conversation = {
        ...
        messages: messages,
        summary: `Fight Club session with ${messages.length - 1} messages`
      };
  ...
  const handleSendMessage = () => {
    if (newMessage.trim()) {
      ...
      setTimeout(() => {
        const responses = [
          "My character would absolutely destroy yours! 💪",
          "That's actually a good point... but mine still wins! 🤔",
          "Plot armor would save my character! 📖",
          "Experience points give mine the edge! 🎮",
          "My character has better stats! 📊",
          "That's unfair! My character is OP! ⚡"
        ];
  ...
  return (
    <div className={`flex flex-col h-screen bg-gradient-to-b from-red-50 via-orange-50 to-yellow-50 transition-opacity duration-500 ${
      isVisible ? "opacity-100" : "opacity-0"
    }`}>
      {/* Header */}
      <div className="bg-gradient-to-r from-red-500 via-orange-500 to-yellow-600 text-white p-4 shadow-lg">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="font-bold text-lg">Fight Club</h1>
                <p className="text-sm opacity-90">Character battles!</p>
              </div>
            </div>
          </div>
            ...
            >
              <div
                className={`max-w-xs lg:max-w-md px-4 py-2 rounded-2xl shadow-md ${
                  message.sender === 'user'
                    ? 'bg-gradient-to-r from-red-500 via-orange-500 to-yellow-600 text-gray-900 rounded-br-sm'
        ...
        <div className="flex items-end gap-2">
          ...
          <div className="flex-1 relative">
            <textarea
              ...
              placeholder="Who would win in a fight?..."
          ...
          <button
            ...
            className={`p-3 rounded-full transition-all duration-200 ${
              newMessage.trim()
                ? 'bg-gradient-to-r from-red-500 via-orange-500 to-yellow-600 text-white shadow-lg hover:shadow-xl'
