...
import { Radio, Flame, Zap, Music } from "lucide-react";

export default function HotTakesFmAnim() {
  ...
  useEffect(() => {
    ...
    const timer4 = setTimeout(() => navigate("/games/hottakesfm/chat"), 4000);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-orange-900 via-red-800 to-pink-700 flex items-center justify-center relative overflow-hidden">
      ...
      {/* Floating radios, flames, and musical notes */}
      <div className="absolute inset-0">
        {[...Array(12)].map((_, i) => (
          <div
            key={`decor-${i}`}
          ...
          >
            {i % 4 === 0 ? <Radio size={24} className="text-orange-400" /> :
             i % 4 === 1 ? <Flame size={24} className="text-red-400" /> :
             i % 4 === 2 ? <Music size={24} className="text-pink-400" /> :
      ...
      <div className="text-center z-10">
        ...
        <div className={`transition-all duration-1000 ${animationStep >= 1 ? 'scale-100 opacity-100' : 'scale-0 opacity-0'}`}>
          <div className="relative mb-8">
            <div className="flex items-center justify-center gap-8">
              ...
              <Flame size={80} className="text-red-400 animate-pulse" />
            </div>
            <div className="absolute -top-4 -right-4">
              <Zap size={40} className="text-orange-400 animate-spin" />
            </div>
          </div>
        </div>
        ...
        <div className={`transition-all duration-1000 ${animationStep >= 2 ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'}`}>
          ...
          <div className="flex justify-center items-center gap-4 mb-8">
            <Radio size={40} className="text-orange-400 animate-ping" />
            <span className="text-2xl md:text-3xl font-bold text-pink-200">RADIO STATION</span>
            <Flame size={40} className="text-red-400 animate-ping" />
          </div>
        </div>
        ...
        <div className={`transition-all duration-1000 ${animationStep >= 3 ? 'scale-100 opacity-100' : 'scale-75 opacity-0'}`}>
          <p className="text-xl md:text-2xl text-orange-200 mb-8 font-semibold">
            📻 Tune into controversial music opinions! 📻
          </p>

          {/* Loading animation */}
          <div className="flex justify-center items-center gap-2">
            <div className="w-3 h-3 bg-orange-300 rounded-full animate-bounce" style={{ animationDelay: '0s' }}></div>
            ...
            <span className="text-white font-bold ml-4">Tuning into the station...</span>
          </div>
        </div>
      </div>
      ...
      <div className="absolute top-16 left-8 animate-bounce" style={{ animationDelay: '1s' }}>
        <div className="bg-white rounded-2xl p-3 shadow-lg relative">
          <div className="text-sm font-bold text-gray-800">This opinion is fire! 🔥</div>
      ...
      <div className="absolute top-28 right-8 animate-bounce" style={{ animationDelay: '1.5s' }}>
        <div className="bg-white rounded-2xl p-3 shadow-lg relative">
          <div className="text-sm font-bold text-gray-800">Wait, what?! 🤯</div>
      ...
      <div className="absolute bottom-16 left-16 animate-bounce" style={{ animationDelay: '2s' }}>
        <div className="bg-white rounded-2xl p-3 shadow-lg relative">
          <div className="text-sm font-bold text-gray-800">Love it or hate it! ❤️💀</div>
          <div className="absolute -bottom-2 left-4 w-0 h-0 border-l-4 border-r-4 border-t-4 border-l-transparent border-r-transparent border-t-white"></div>
        </div>
      </div>
    </div>
